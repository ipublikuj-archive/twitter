 ...
 */
class CurlClient extends Nette\Object implements Twitter\HttpClient
{
	/**
	...
	public static $defaultCurlOptions = [
		...
		CURLOPT_TIMEOUT => 20,
		CURLOPT_USERAGENT => 'ipub-twitter-php',
		CURLOPT_HTTPHEADER => [
	...
	public function makeRequest(Request $request)
	{
		...
		if (curl_errno($ch) == CURLE_SSL_CACERT || curl_errno($ch) === CURLE_SSL_CACERT_BADFILE) {
			Debugger::log('Invalid or no certificate authority found, using bundled information', 'twitter');
		...
		if ($result === FALSE && (!isset($this->curlOptions[CURLOPT_IPRESOLVE]) || empty($this->curlOptions[CURLOPT_IPRESOLVE]))) {
			...
			if (preg_match('/Failed to connect to ([^:].*): Network is unreachable/', curl_error($ch), $matches)) {
				if (strlen(@inet_pton($matches[1])) === 16) {
					Debugger::log('Invalid IPv6 configuration on server, Please disable or get native IPv6 on your server.', 'twitter');
